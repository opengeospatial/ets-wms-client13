        ...
        <ctl:code>
            ...
            <xsl:if test="$styles != ''">
                ...
                <xsl:if test="count($layer-values/value) ne count($style-values/value)">
                    <ctl:message>
                        <xsl:value-of select="count($style-values/value)"/> styles, <xsl:value-of select="count($layer-values/value)"/> layers</ctl:message>
                    <ctl:fail/>
                </xsl:if>
            </xsl:if>
        </ctl:code>
    ...
    <ctl:test name="gm:format">
        ...
        <ctl:code>
            ...
            <xsl:if test="not($getmap-element/wms:Format[.=$value])">
                <ctl:message>
                    <xsl:value-of select="$value"/> is not a valid FORMAT value.</ctl:message>
                <ctl:fail/>
            </xsl:if>
        </ctl:code>
    </ctl:test>
    ...
    <ctl:test name="gm:width-height">
        ...
        <ctl:code>
            ...
            <xsl:if test="contains($value, '.') or string(number($value)) = 'NaN' or number($value) le 0">
                <ctl:message>
                    <xsl:value-of select="$value"/> is not a positive integer.</ctl:message>
                <ctl:fail/>
            </xsl:if>
            <xsl:if test="$limit != ''">
                <xsl:if test="number($value) gt number($limit)">
                    <ctl:message>
                        <xsl:value-of select="$value"/> exceeds the declared Max<xsl:value-of select="$param"/> value of <xsl:value-of select="$limit"/>.</ctl:message>
                    <ctl:fail/>
                </xsl:if>
            </xsl:if>
        </ctl:code>
    ...
    <ctl:test name="gm:transparent">
        ...
        <ctl:code>
            ...
            <xsl:if test="$value != 'TRUE' and $value != 'FALSE'">
                <ctl:message>
                    <xsl:value-of select="$value"/> is an invalid value for TRANSPARENT.</ctl:message>
                <ctl:fail/>
            </xsl:if>
        </ctl:code>
    </ctl:test>
    ...
    <ctl:test name="gm:exceptions">
        ...
        <ctl:code>
            ...
            <xsl:if test="not($exception-element/wms:Format[.=$value])">
                <ctl:message>
                    <xsl:value-of select="$value"/> is not a valid EXCEPTIONS value.</ctl:message>
                <ctl:fail/>
            </xsl:if>
        </ctl:code>
    </ctl:test>
